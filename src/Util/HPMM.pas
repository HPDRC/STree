...
const
  ...
  MinTableIndex=5;
  MaxTableIndex=31; // = 4 GB max allocation.
...
type
  ...
  public
    ...
    PrevIndex:byte;
    UsedSpace:byte;
    procedure SetSize(value,AlignedSize:integer);
    procedure MakeHighRemnant;
    function Size:integer;
    function Prev:PMemBlock;
    function Next:PMemBlock;
    function IsValid:boolean;
  end;

  PFreeBlock=^TFreeBlock;
  TFreeBlock=object(TMemBlock)
  public
    NextBlock:PFreeBlock;
    procedure Init(aSizeIndex: integer; PrevBlock: PMemBlock);
  end;
  ...
  public
    ...
    function HiRemnant:PMemBlock;
    function LoRemnant:PMemBlock;
  end;
...
function TMemBlock.IsValid: boolean;
begin // 22.5 bits of error checking
  Result:=(SizeValidTable[SizeIndex] and PrevValidTable[prevIndex] and byte(BlockType))=1;
end;

function TMemBlock.Next: PMemBlock;
begin
//  if BlockType<>btHiRemnant then
    result:=Pointer(DWord(@Self)+DWord(1 shl SizeIndex))
//  else
//    result:=Pointer(DWord(@Self)+4);
end;

function TMemBlock.Prev: PMemBlock;
begin
  if PrevIndex<>LoRemnantFlag then
    result:=Pointer(DWord(@Self)-DWord(1 shl PrevIndex))
  else
    result:=Pointer(DWord(@Self)-(MinAlignedSize-4));
end;

function TMemBlock.Size: integer;
begin
  if BlockType<=btUsed then
    if SizeIndex>7 then
      Result:=UsedSpace shl (SizeIndex-7)
    else
      Result:=UsedSpace
  else if BlockType=btLoRemnant then
    Result:=MinAlignedSize - 4
  else
    Result:=4;
end;

procedure TMemBlock.SetSize(value,AlignedSize:integer);
var
  shiftIndex:integer;
begin
  shiftIndex:=sizeIndex-7;
  if shiftIndex>0 then
  begin
    value:=value + (AlignedSize-1) shr 7;
    UsedSpace:=value shr shiftIndex;
  end
  else
    UsedSpace:=value;
end;

procedure TMemBlock.MakeHighRemnant;
begin
...
procedure TFreeBlock.Init(aSizeIndex: integer; PrevBlock: PMemBlock);
type
  PInteger=^Integer;
begin
  PInteger(@Self)^:=aSizeIndex;
  PrevIndex:=PrevBlock.SizeIndex;
end;
...
function TAlloc.HiRemnant: PMemBlock;
begin
  Result:=Pointer(DWord(@Self)+Size-SizeOf(TMemBlock));
end;

procedure TAlloc.init(aSize: integer);
const
...
begin
  Size:=aSize;
  LoRemnantBlock:=LoRemnantConst;
  HiRemnant^:=HiRemnantConst;
  Next:=Pointer(LastBlockFlag);
end;

function TAlloc.LoRemnant: PMemBlock;
begin
  Result:=@LoRemnantBlock;
end;
...
//  assert(Index>0,'Invalid Resize');
  ...
  begin
    Block:=Block.Next;
    PFreeBlock(Block).init(NewSizeIndex,PrevBlock);
  ...
  begin
    Block:=Block.Next;
    PFreeBlock(Block).Init(i,PrevBlock);
    ReturnToPool(PFreeBlock(Block));
    PrevBlock:=Block;
  end;
...
begin
  ...
  Block.SetSize(Size,1 shl SizeIndex);
...
begin
  ...
  begin
    ...
    begin
      if NewSizeIndex+ToleratedShrinkIndex>HighOrderBit(Block.Size) then
...
{$endif}
    ...
    if NewSizeIndex<SizeIndex then
      ...
      MoveMem32(Result,Addr,(Block.Size-SizeOf(DWord)+3) shr 2); // all orig data
