// GPC.cpp : Defines the entry point for the DLL application.
//
...
static edge_node **bound_list(lmt_node **lmt, float y)
{
  ...
  if (!*lmt)
  {
    ...
    MALLOC(*lmt, sizeof(lmt_node), "LMT insertion", lmt_shape*);
    (*lmt)->y= y;
...
static void add_to_sbtree(int *entries, sb_tree **sbtree, float y)
{
  if (!*sbtree)
  {
    ...
    MALLOC(*sbtree, sizeof(sb_tree), "scanbeam tree insertion", sbt_t_shape*);
    (*sbtree)->y= y;
    (*sbtree)->less= NULL;
    (*sbtree)->more= NULL;
    (*entries)++;
                            ...
                            gpc_op op)
{
  ...
  MALLOC(edge_table, total_vertices * sizeof(edge_node),
         "edge table creation", edge_shape*);
                             ...
                             float x, float y)
{
  ...
  if (!*it)
  {
    ...
    MALLOC(*it, sizeof(it_node), "IT insertion", it_shape*);
    (*it)->ie[0]= edge0;
    (*it)->ie[1]= edge1;
    (*it)->point.x= x;
    (*it)->point.y= y;
                        ...
                        float dy)
{
  ...
  if (!*st)
  {
    ...
    MALLOC(*st, sizeof(st_node), "ST insertion", st_shape*);
    (*st)->edge= edge;
    (*st)->xb= edge->xb;
    (*st)->xt= edge->xt;
    (*st)->dx= edge->dx;
...
static void add_left(polygon_node *p, float x, float y)
{
  ...
  MALLOC(nv, sizeof(vertex_node), "vertex node creation", v_shape*);
  nv->x= x;
  nv->y= y;
                          ...
                          float x, float y)
{
  ...
  MALLOC(*p, sizeof(polygon_node), "polygon node creation", p_shape*);
...
static void add_vertex(vertex_node **t, float x, float y)
{
  if (!(*t))
  {
    MALLOC(*t, sizeof(vertex_node), "tristrip vertex creation", v_shape *);
    (*t)->x= x;
    (*t)->y= y;
    (*t)->next= NULL;
  }
                         ...
                         float x, float y)
{
  if (!(*tn))
  {
    MALLOC(*tn, sizeof(polygon_node), "tristrip node creation", p_shape *);
    (*tn)->next= NULL;
    (*tn)->v[LEFT]= NULL;
    (*tn)->v[RIGHT]= NULL;
    (*tn)->active= 1;
...
static bbox *create_contour_bboxes(gpc_polygon *p)
{
  ...
  MALLOC(box, p->num_contours * sizeof(bbox), "Bounding box creation", bbox_shape*);
...
extern "C"
GPC_API void gpc_free_polygon(gpc_polygon *p)
{
  int c;
...
extern "C"
GPC_API void gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
                      gpc_polygon *result)
{
  sb_tree       *sbtree= NULL;
